...
$PAGE->set_url('/moodle/plagiarism/plagkhal/plagiarism_plagkhal_report.php', array(
    'cmid' => $cmid,
    'userid' => $userid,
...
if ($file) {
    ...
    $pagetitle = get_string('clreportpagetitle', 'plagiarism_plagkhal') . ' - ' . fullname($user) . ' - ' . $filename;
} else {
    $pagetitle = get_string('clreportpagetitle', 'plagiarism_plagkhal') . ' - ' . fullname($user);
}
...
// plagkhal course settings.
$modulesettings = $DB->get_records_menu('plagiarism_plagkhal_config', array('cm' => $cmid), '', 'name,value');

$isinstructor = plagiarism_plagkhal_assignmodule::is_instructor($context);


$clmoduleenabled = plagiarism_plagkhal_pluginconfig::is_plugin_configured('mod_' . $cm->modname);

// Check if plagkhal plugin is disabled.
if (empty($clmoduleenabled) || empty($modulesettings['plagiarism_plagkhal_enable'])) {
    echo html_writer::div(get_string('cldisabledformodule', 'plagiarism_plagkhal'), null, array('style' => $errormessagestyle));
} else {
    // Incase students not allowed to see the plagiairsm score.
    if (!$isinstructor && empty($modulesettings['plagiarism_plagkhal_allowstudentaccess'])) {
        echo html_writer::div(get_string('clnopageaccess', 'plagiarism_plagkhal'), null, array('style' => $errormessagestyle));
    } else {
        ...
        if ($isinstructor || in_array($USER->id, $owners)) {
            ...
            if ($viewmode == 'course') {
                echo html_writer::link(
                    ...
                    get_string('clopenfullscreen', 'plagiarism_plagkhal'),
                    array('title' => get_string('clopenfullscreen', 'plagiarism_plagkhal'))
                );
            }

            $cl = new plagiarism_plagkhal_comms();
            $scanaccesstoken = $cl->request_access_for_report($plagiarismfiles->externalid, $isinstructor);
            $lang = plagiarism_plagkhal_utils::get_lang();
            echo html_writer::tag(
                'iframe',
                null,
                array(
                    'title' => 'plagkhal Report',
                    'srcdoc' =>
                    "<form target='_self'" .
                        "method='POST'" .
                        "style='display: none;'" .
                        "action='$config->plagiarism_plagkhal_apiurl/api/moodle/$config->plagiarism_plagkhal_key" .
