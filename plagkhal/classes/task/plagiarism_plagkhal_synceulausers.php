 ...
 * plagkhal Plagiarism Plugin - Handle Resubmit Files
 */
class plagiarism_plagkhal_synceulausers extends \core\task\scheduled_task {
    /**
     * Get scheduler name, this will be shown to admins on schedulers dashboard.
     */
    public function get_name() {
        return get_string('clupserteulausers', 'plagiarism_plagkhal');
    }
    ...
    private function handle_synced_users() {
        ...
        while ($canloadmoredata && (--$maxdataloadloops) > 0) {
            try {
                ...
                $canloadmoredata = $recordscount == PLAGIARISM_plagkhal_CRON_QUERY_LIMIT;
            ...
            if (count($useridstosync) > 0) {
                // Get only the users that theirs ids is return from the plagkhal server.
                ...
                foreach ($eulatosyncarray as $eulauser) {
                    ...
                    if (!$DB->update_record('plagiarism_plagkhal_eula', $eulauser)) {
                        \plagiarism_plagkhal_logs::add(
            ...
            $limitfrom = $limitfrom + PLAGIARISM_plagkhal_CRON_QUERY_LIMIT;
        }
    }
