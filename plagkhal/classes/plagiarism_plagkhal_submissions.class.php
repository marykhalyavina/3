 ...
 * submissions helpers methods
 */
class plagiarism_plagkhal_submissions {
    /**
    ...
    public static function create($cm, $userid, $identifier, $submissiontype) {
        ...
        if (!$fileid = $DB->insert_record('plagiarism_plagkhal_files', $file)) {
    ...
    public static function save(
        ...
        if ($fileid != 0) {
            if (!$DB->update_record('plagiarism_plagkhal_files', $file)) {
                \plagiarism_plagkhal_logs::add(
            ...
            if (!$DB->insert_record('plagiarism_plagkhal_files', $file)) {
                \plagiarism_plagkhal_logs::add(
    ...
    public static function mark_error($fileid, $errormsg = null) {
        ...
        if (!$DB->update_record('plagiarism_plagkhal_files', $file)) {
            ...
            \plagiarism_plagkhal_logs::add($errormsg . " (fileid: " . $fileid . ") - ", "MARK_ERROR_SUBMISSION");
        }
    ...
    /**
     * check if submission already submitted, to avoid resubmitting them to plagkhal.
     * @param string $cmid
     * @param string $author
     * @param string $identifier
