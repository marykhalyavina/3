 ...
 * submission display helpers methods
 */
class plagiarism_plagkhal_submissiondisplay {
    /**
    ...
    public static function output($submissionref) {
        ...
        // Get plagkhal plugin admin config.
        static $adminconfig;
        if (empty($adminconfig)) {
            $adminconfig = plagiarism_plagkhal_pluginconfig::admin_config();
        }

        // Is plagkhal plugin enabled for this module type?
        static $ismodenabledforcl;
        if (empty($ismodenabledforcl)) {
            $moduleconfigname = 'plagiarism_plagkhal_mod_' . $coursemodule->modname;
        ...
        $enabledproperty = 'plagiarism_plagkhal_enable';
        ...
        if (empty($isinstructor)) {
            $isinstructor = plagiarism_plagkhal_assignmodule::is_instructor($ctx);
        }

        // Incase of students, check if he is allowed to view the plagiairsm report progress & results.
        if (!$isinstructor && empty($clmodulesettings['plagiarism_plagkhal_allowstudentaccess'])) {
            return;
        }
        ...
        if (!empty($submissionref["cmid"] && (!empty($submissionref["content"]) || !empty($submissionref["file"])))) {
            ...
            */
            ...
            if ((!empty($assignsubmission->groupid)) && ($coursemodule->modname == "assign")) {
                ...
                if ($subitemid != 0) {
                    $author = plagiarism_plagkhal_assignmodule::get_author($subitemid);
            ...
            if ($isinstructor || in_array($USER->id, $submissionusers)) {
                ...
                if ($submittedfile) {
                    $clpoweredbyplagkhaltxt = get_string('clpoweredbyplagkhal', 'plagiarism_plagkhal');
                    switch ($submittedfile->statuscode) {
                        case 'success':
                            ...
                            $results['reporturl'] =
                                "$CFG->wwwroot/plagiarism/plagkhal/plagiarism_plagkhal_report.php" .
                            ...
                            $similaritywrapper = '<a class="plagkhal-text-gray" title="'
                                . get_string('clopenreport', 'plagiarism_plagkhal') . '" href="'
                                . $results['reporturl'] . '" target="_blank">';

                            $similaritywrapper .= get_string('clplagiarised', 'plagiarism_plagkhal') . ':'
                                . $similaritystring . '</a>';

                            /*$divcontent = $OUTPUT->pix_icon(
                                'plagkhal-logo',
                                $clpoweredbyplagkhaltxt,
                                'plagiarism_plagkhal',
                            ...
                            $output = html_writer::tag(
                                'div',
                                $similaritywrapper,
                                array('class' => 'plagkhal')
                            );

                            if ($submittedfile->ischeatingdetected) {
                                $cheatingdetectioncontent = '<a class="plagkhal-text-warn" title="'
                                    . get_string('clcheatingdetected', 'plagiarism_plagkhal')
                                    . '" href="'
                                    . $results['reporturl'] . '" target="_blank">'
                                    . get_string('clcheatingdetectedtxt', 'plagiarism_plagkhal')
                                    . '</a>';
                                $output .= html_writer::tag(
                                    'div',
                        ...
                        case 'error':
                            if ($isinstructor) {
                                ...
                                $clplagiarised = get_string('clplagiarised', 'plagiarism_plagkhal');
                                $errorstring = '&nbsp;<span class="plagkhal-text-gray">'
                                    . $clplagiarised . ':&nbsp;</span>&nbsp;<span class="strong">'
                                    . get_string('clplagiarisefailed', 'plagiarism_plagkhal')
                                    . '</span>&nbsp;';
                        ...
                        case 'pending':
                            ...
                            $pendingstring = '&nbsp;<span class="plagkhal-text-gray">'
                                . $clplagiarised . ':&nbsp;</span>';

                            $pendingwrapper = '<span title="' . get_string('clplagiarisescanning', 'plagiarism_plagkhal') . '">'
                                . $pendingstring . '</span>';
                            ...
                            /*$output = html_writer::tag(
                                ...
                                $OUTPUT->pix_icon(
                                    ...
                                    . $OUTPUT->pix_icon(
                                        'plagkhal-loading',
                                        get_string('clplagiarisescanning', 'plagiarism_plagkhal'),
                                        'plagiarism_plagkhal',
                                        array('class' => 'icon_size')
                                    ),
                                array('class' => 'plagkhal')
                        ...
                        case 'queued':
                            ...
                            $queuedstring = '&nbsp;<span class="plagkhal-text-gray">'
                                . $clplagiarised . ':&nbsp;</span>';
