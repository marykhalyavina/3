 ...
 */
class plagiarism_plagkhal_dbutils {
    ...
    public static function queued_failed_request($cmid, $endpoint, $data, $priority, $error, $verb, $requireauth = true) {
        ...
        if (!$records) {
            ...
            $request->status = plagiarism_plagkhal_request_status::FAILED;
            $request->fail_message = $error;
            $request->verb = $verb;
            $request->require_auth = $requireauth;
            if (!$DB->insert_record('plagiarism_plagkhal_request', $request)) {
     ...
     */
    public static function update_plagkhal_eula_version($version) {
        global $DB;
        $configeula = $DB->get_record(
            'plagiarism_plagkhal_config',
            array('cm' => PLAGIARISM_plagkhal_DEFAULT_MODULE_CMID, 'name' => PLAGIARISM_plagkhal_EULA_FIELD_NAME)
        );

        if ($configeula) {
            $configeula->value = $version;
            if (!$DB->update_record('plagiarism_plagkhal_config', $configeula)) {
                \plagiarism_plagkhal_logs::add(
            ...
            $configeula = array(
                ...
                'name' => PLAGIARISM_plagkhal_EULA_FIELD_NAME,
                'value' => $version,
                'config_hash' => PLAGIARISM_plagkhal_DEFAULT_MODULE_CMID . "_" . PLAGIARISM_plagkhal_EULA_FIELD_NAME
            );
            if (!$DB->insert_record('plagiarism_plagkhal_config', $configeula)) {
    ...
    public static function is_user_eula_uptodate($userid) {
        ...
        $user = $DB->get_record('plagiarism_plagkhal_users', array('userid' => $userid));
        if (!$user || !isset($user)) {
            return false;
        }

        $version = self::get_plagkhal_eula_version();
        return self::is_eula_version_update_by_userid($userid, $version);
    }

    /**
     * Update in plagkhal server that the user accepted the current version.
     * @param string userid
     */
    public static function upsert_eula_by_user_id($userid) {
        global $DB;
        $user = $DB->get_record('plagiarism_plagkhal_users', array('userid' => $userid));
        $curreulaversion = self::get_plagkhal_eula_version();

        if (!$user) {
            if (!$DB->insert_record('plagiarism_plagkhal_users', array('userid' => $userid))) {
                \plagiarism_plagkhal_logs::add(
                    "failed to insert new database record for : " .
                        "plagiarism_plagkhal_users, Cannot create new user record for user $userid",
                    "INSERT_RECORD_FAILED"
                );
            }
        }
        ...
        if (
            ...
            && !$DB->insert_record('plagiarism_plagkhal_eula', $newusereula)
        ) {
            \plagiarism_plagkhal_logs::add(
                "failed to insert new database record for :" .
                    "plagiarism_plagkhal_eula, Cannot create new user record eula for user $userid",
                "INSERT_RECORD_FAILED"
            );
        }
    }
     ...
     * return string
     */
    public static function get_plagkhal_eula_version() {
        global $DB;
        ...
        return PLAGIARISM_plagkhal_DEFUALT_EULA_VERSION;
    }
