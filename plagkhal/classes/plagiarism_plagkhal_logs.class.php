...
/**
 * admins logs helper for plagkhal plugin
 ...
 * admins logs helper for plagkhal plugin
 */
class plagiarism_plagkhal_logs {
    /**
     * add log message
    ...
    public static function add($errormessage, $code) {
        ...
        $directorypath = self::temp_dir_path() . "/plagiarism_plagkhal/error_logs";
        ...
        while ($file = readdir($directoryref)) {
            ...
            if (substr(basename($file), 0, 1) != "." && substr_count(basename($file), PLAGIARISM_plagkhal_LOGS_PREFIX) > 0) {
        ...
        // Write to log file.
        $newfilepath = $directorypath . "/" . PLAGIARISM_plagkhal_LOGS_PREFIX . gmdate('Y-m-d', time()) . ".txt";
    ...
    public static function displaylogs($date = null) {
        ...
        if (is_null($date)) {
                ...
                while (false !== ($file = readdir($readerrdir))) {
                    if (substr_count($file, 'log') > 0) {
                        ...
                        echo $OUTPUT->box(html_writer::link(
                            $CFG->wwwroot . '/plagiarism/plagkhal/settings.php?tab=plagkhallogs' . '&date=' . $date,
                            $displayedfilename,
                            array('target' => '_blank')
                        ), '');
                    }
            ...
            $file = PLAGIARISM_plagkhal_LOGS_PREFIX . $date . '.txt';
