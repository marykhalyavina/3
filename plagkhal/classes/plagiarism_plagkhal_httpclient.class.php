...
/**
 * plagkhal_httpclient.class.php - containes a generic method for AJAX calls
 ...
 */
class plagiarism_plagkhal_http_client {
    ...
    public static function execute(
        ...
        $c->setopt(
            array(
                ...
                'CURLOPT_TIMEOUT' => 70, // Set to 70 seconds according to the plagkhal API docs.
        ...
        if ($requireauth) {
            $cljwttoken = plagiarism_plagkhal_comms::login_to_plagkhal();
        ...
        if (self::is_success_status_code($statuscode)) {
            ...
            if (!$isauthretry) {
                // Try to get the jwt again from plagkhal (retry if unauthorized).
                $cljwttoken = plagiarism_plagkhal_comms::login_to_plagkhal(null, null, null, true);
                if (isset($cljwttoken)) {
                    return self::execute($verb, $url, $requireauth, $data, true);
                }
            }
            throw new plagiarism_plagkhal_auth_exception();
        } else if (self::is_under_maintenance_response($statuscode)) {
            throw new plagiarism_plagkhal_under_maintenance_exception();
        } else if (self::is_rate_limit_response($statuscode)) {
            throw new plagiarism_plagkhal_rate_limit_exception();
        } else {
            throw new plagiarism_plagkhal_exception($result, $statuscode);
        }
    }
    ...
    public static function execute_retry(
        ...
        $retrycnt = 0;
        $maxretries = count(PLAGIARISM_plagkhal_RETRY);
        $serverresult = null;

        while (!isset($serverresult) && $retrycnt < $maxretries) {
            try {
                sleep(PLAGIARISM_plagkhal_RETRY[$retrycnt]);
                ...
                return $serverresult;
            } catch (plagiarism_plagkhal_under_maintenance_exception $ume) {
                throw $ume;
            } catch (plagiarism_plagkhal_rate_limit_exception $rle) {
                throw $rle;
            } catch (Exception $e) {
