 ...
 * handle observer events
 */
class plagiarism_plagkhal_eventshandler {
    ...
    public function handle_submissions($data) {
        ...
        if (!plagiarism_plagkhal_moduleconfig::is_module_enabled($coursemodule->modname, $coursemodule->id)) {
            return true;
        }

        // Check the supported EULA acceptance module.
        if (plagiarism_plagkhal_moduleconfig::is_allowed_eula_acceptance($coursemodule->modname)) {
            plagiarism_plagkhal_dbutils::upsert_eula_by_user_id($data["userid"]);
        }
        ...
        $clmoduleconfig = plagiarism_plagkhal_moduleconfig::get_module_config($coursemodule->id);

        if ($coursemodule->modname == 'assign') {
            // Default to 0 => Submit file when first uploaded.
            $clmoduleconfig["plagiarism_plagkhal_draftsubmit"] = ($cmdata->submissiondrafts &&
                isset($clmoduleconfig["plagiarism_plagkhal_draftsubmit"])) ?
                $clmoduleconfig["plagiarism_plagkhal_draftsubmit"] : 0;
        }
        ...
        if (
            ...
            $clmoduleconfig["plagiarism_plagkhal_draftsubmit"] == 1
        ) {
            return true;
        }
    ...
    /**
     * queue text content to be send to plagkhal later by scan submission task.
     * @param object $data
    ...
    private function queue_text_content($data, $coursemodule, $authoruserid, $submitteruserid, $cmdata) {
        ...
        $files = plagiarism_plagkhal_submissions::successful_submission_instances(
            $coursemodule->id,
            $authoruserid,
            $contentidentifier
        );
        if (count($files) > 0) {
            ...
            return $this->queue_submission_to_plagkhal(
                $coursemodule,
                $authoruserid,
                $submitteruserid,
                $contentidentifier,
    ...
    /**
     * queue quizzes to be send to plagkhal later by scan submission task.
     * @param object $data
     * @param object $coursemodule
     * @param string $authoruserid
     * @param string $submitteruserid
    ...
    private function queue_quizzes($data, $coursemodule, $authoruserid, $submitteruserid, $cmdata) {
        ...
        foreach ($attempt->get_slots() as $slot) {
            ...
            // Queue text to plagkhal.
            $identifier = sha1($data['other']['content']);
            $result = $this->queue_submission_to_plagkhal(
                $coursemodule,
                $authoruserid,
                $submitteruserid,
                $identifier,
    ...
    /**
     * queue files to be send to plagkhal later by scan submission task.
     * @param object $data
     * @param object $coursemodule
     * @param string $authoruserid
     * @param string $submitteruserid
    ...
    /**
     * Queue submission to be submitted later to plagkhal by submission task.
     * @param object $coursemodule
     * @param string $authoruserid
     * @param string $submitteruserid
     ...
     * @return bool
     */
    private function queue_submission_to_plagkhal(
        $coursemodule,
        $authoruserid,
        $submitteruserid,
        $identifier,
        ...
        if ($DB->get_records_select(
            ...
            $submissionid = plagiarism_plagkhal_submissions::create($coursemodule, $authoruserid, $identifier, $subtype);
        }

        // Check if file type is supported by plagkhal.
        if ($subtype == 'file') {
            $parts = explode('.', $filename);
            $extension = strtolower(end($parts));
            if (!in_array(
                "." . $extension,
                PLAGIARISM_plagkhal_ACCEPTED_FILES
            )) {
        ...
        if ($subtype == 'file') {
            if ($fileref->get_filesize() > PLAGIARISM_plagkhal_MAX_FILE_UPLOAD_SIZE) {
        ...
        if (isset($cmdata->duedate)) {
            ...
            if ($clmoduleconfig["plagiarism_plagkhal_reportgen"]) {
                // Scan on due date.
                $scheduledscandate = $cmdata->duedate - (1 * 60);
            }
        }

        // If we have error message, then we don't need to send it to plagkhal.
        $submitstatus = $errormessage == null ? 'queued' : 'error';
        return plagiarism_plagkhal_submissions::save(
            $coursemodule,
            $authoruserid,
            $submissionid,
            $identifier,
    ...
    public function handle_user_deletion($data) {
        global $DB;
        $usertable = "plagiarism_plagkhal_users";
        $userid = $data['objectid'];

        // Check if the user not agreed already.
        $isuseragreed = $DB->record_exists("plagiarism_plagkhal_users", array('userid' => $userid));
        ...
        $isuseragreed = $DB->record_exists("plagiarism_plagkhal_eula", array('ci_user_id' => $userid));
        if ($isuseragreed) {
            if (!($DB->delete_records('plagiarism_plagkhal_eula', ['ci_user_id' => $userid]))) {
                \plagiarism_plagkhal_logs::add(
                    "Faild to delete row in 'plagiarism_plagkhal_eula'. (user id " . $userid,
                    "DELETE_RECORD_FAILED"
                );
            };
        }
    ...
    public function handle_eula_acceptance($data, $isretry = false) {
        ...
        $usertable = "plagiarism_plagkhal_users";
        // Check if the user not agreed already.
        $isuseragreed = $DB->record_exists("plagiarism_plagkhal_users", array('userid' => $data["userid"]));
