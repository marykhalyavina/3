 ...
 * module configurations helpers methods
 */
class plagiarism_plagkhal_moduleconfig {
    /**
    ...
    public static function get_modules_default_config() {
        ...
        if (count($result) > 0) {
            $result['cmid'] = PLAGIARISM_plagkhal_DEFAULT_MODULE_CMID;
        }
        return $result;
    }
    ...
    public static function set_module_config(
        $cmid = PLAGIARISM_plagkhal_DEFAULT_MODULE_CMID,
        $enabled = false,
        $draftssubmit = 0,
        $reportgen = 0,
        ...
        $default = array();
        $default['plagiarism_plagkhal_enable'] = $enabled;
        $default['plagiarism_plagkhal_draftsubmit'] = $draftssubmit;
        $default['plagiarism_plagkhal_reportgen'] = $reportgen;
        $default['plagiarism_plagkhal_allowstudentaccess'] = $allowstudentaccess;

        // Db settings elements name.
        $clcmconfigfields = self::get_config_db_properties();

        // Get saved db settings.
        $saveddefaultvalue = $DB->get_records_menu('plagiarism_plagkhal_config', array('cm' => $cmid), '', 'name,value');
    ...
    /**
     * Check if plagkhal plugin is enabled
     * @param string $modulename course module name
     * @param string $cmid course module id
     * @return bool is plagkhal plugin enabled
     */
    public static function is_module_enabled($modulename, $cmid) {
        $plagiarismsettings = self::get_module_config($cmid);

        $moduleclenabled = plagiarism_plagkhal_pluginconfig::is_plugin_configured('mod_' . $modulename);
        if (empty($plagiarismsettings['plagiarism_plagkhal_enable']) || empty($moduleclenabled)) {
            return false;
        }
        return true;
    }
    ...
    public static function is_course_module_request_queued($cmid) {
        global $DB;
        $record = $DB->get_record('plagiarism_plagkhal_request', ['cmid' => $cmid]);
