 ...
 * @return bool
 */
function xmldb_plagiarism_plagkhal_upgrade($oldversion) {
    global $DB;
    $dbman = $DB->get_manager();
    if ($oldversion < 2023090901) {
        // Changing type of field similarityscore on table plagiarism_plagkhal_files to number.
        $table = new xmldb_table('plagiarism_plagkhal_files');
        ...
        // plagkhal savepoint reached.
        upgrade_plugin_savepoint(true, 2023090901, 'plagiarism', 'plagkhal');
    }
    ...
    if ($oldversion < 2022072100) {
        ...
        $saveddefaultvalue = $DB->get_records_menu(
            ...
            array('cm' => PLAGIARISM_plagkhal_DEFAULT_MODULE_CMID),
            '',
            'name,value'
        );

        // Update saved default plagkhal settings.
        $fieldname = 'plagiarism_plagkhal_enable';
        $savedfield = new stdClass();
        $savedfield->cm = PLAGIARISM_plagkhal_DEFAULT_MODULE_CMID;
        $savedfield->name = $fieldname;
        $savedfield->value = 0;
        if (!isset($saveddefaultvalue[$fieldname])) {
            $savedfield->config_hash = $savedfield->cm . "_" . $savedfield->name;
            if (!$DB->insert_record('plagiarism_plagkhal_config', $savedfield)) {
                throw new moodle_exception(get_string('clinserterror', 'plagiarism_plagkhal'));
            }
        } else {
            $savedfield->id = $DB->get_field(
                ...
                'id',
                (array(
                    'cm' => PLAGIARISM_plagkhal_DEFAULT_MODULE_CMID,
                    'name' => $fieldname
                ))
            );
            if (!$DB->update_record('plagiarism_plagkhal_config', $savedfield)) {
                throw new moodle_exception(get_string('clupdateerror', 'plagiarism_plagkhal'));
            }
        }

        upgrade_plugin_savepoint(true, 2022072100, 'plagiarism', 'plagkhal');
    }
    ...
    if ($oldversion < 2022103100) {
        ...
        // Update saved default plagkhal settings.
        $fieldname = 'plagiarism_plagkhal_checkforparaphrase';
        $savedfield = new stdClass();
        $savedfield->cm = PLAGIARISM_plagkhal_DEFAULT_MODULE_CMID;
        $savedfield->name = $fieldname;
        $savedfield->value = 1;
        ...
        upgrade_plugin_savepoint(true, 2022103100, 'plagiarism', 'plagkhal');
    }
    ...
    if ($oldversion < 2022110900) {
        ...
        // Update saved default plagkhal settings.
        $fieldname = 'plagiarism_plagkhal_disablestudentinternalaccess';
        $savedfield = new stdClass();
        $savedfield->cm = PLAGIARISM_plagkhal_DEFAULT_MODULE_CMID;
        $savedfield->name = $fieldname;
        $savedfield->value = 0;
        ...
        upgrade_plugin_savepoint(true, 2022110900, 'plagiarism', 'plagkhal');
    }

    if ($oldversion < 2022122400) {
        $table = new xmldb_table('plagiarism_plagkhal_users');

        // Adding fields to table plagiarism_plagkhal_users.
        ...
        // Adding keys and indexes to table plagiarism_plagkhal_users.
        $table->add_key('primary', XMLDB_KEY_PRIMARY, array('id'));
        $table->add_index('userid', XMLDB_INDEX_UNIQUE, array('userid'));

        // Conditionally launch create table for plagiarism_plagkhal_users.
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }

        upgrade_plugin_savepoint(true, 2022122400, 'plagiarism', 'plagkhal');
    }
    ...
    if ($oldversion < 2022122802) {
        ...
        // Update saved default plagkhal settings.
        $fieldname = 'plagiarism_plagkhal_showstudentresultsinfo';
        $savedfield = new stdClass();
        $savedfield->cm = PLAGIARISM_plagkhal_DEFAULT_MODULE_CMID;
        $savedfield->name = $fieldname;
        $savedfield->value = 0;
        ...
        upgrade_plugin_savepoint(true, 2022122802, 'plagiarism', 'plagkhal');
    }

    if ($oldversion < 2023050701) {
        // Adding fields to table plagiarism_plagkhal_request.
        $table = new xmldb_table('plagiarism_plagkhal_request');
        ...
        // Adding keys and indexes to table plagiarism_plagkhal_request.
        $table->add_key('primary', XMLDB_KEY_PRIMARY, array('id'));

        $table->add_index('created_date', XMLDB_INDEX_NOTUNIQUE, array('created_date'));
        $table->add_index('plagkhal_cmid', XMLDB_INDEX_NOTUNIQUE, array('cmid'));

        // Conditionally launch create table for plagiarism_plagkhal_request.
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }
        ...
        // Add new eula table.
        $table = new xmldb_table('plagiarism_plagkhal_eula');
        $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE);
        ...
        $savedfield = new stdClass();
        $savedfield->cm = PLAGIARISM_plagkhal_DEFAULT_MODULE_CMID;
        $savedfield->name = PLAGIARISM_plagkhal_EULA_FIELD_NAME;
        $savedfield->value = PLAGIARISM_plagkhal_DEFUALT_EULA_VERSION;
        ...
        upgrade_plugin_savepoint(true, 2023050701, 'plagiarism', 'plagkhal');
    }
    return true;
}
