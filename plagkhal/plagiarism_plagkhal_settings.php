...
if ($isadminview) {
    ...
    $PAGE->set_url('/moodle/plagiarism/plagkhal/plagiarism_plagkhal_settings.php');
} else {
    ...
    $PAGE->set_url('/moodle/plagiarism/plagkhal/plagiarism_plagkhal_settings.php', array(
        'courseid' => $courseid,
        'cmid' => $cmid,
...
if (!$isnewmodulesettings && !$isadminview && !$clmoduleenabled) {
    ...
    if (!$isinstructor) {
        ...
        if ($isinstructor) {
            ...
            if (!$isadminview) {
                $breadcrumbs = plagiarism_plagkhal_utils::set_plagkhal_page_navbar_breadcrumbs(
                ...
                $role = 1;
                $breadcrumbs = plagiarism_plagkhal_utils::set_plagkhal_page_navbar_breadcrumbs(null, null);
                $accesstoken = $cl->request_access_for_settings($role, $breadcrumbs, $modulename, null, $cmid);
            }

            $lang = plagiarism_plagkhal_utils::get_lang();

            $actionurl = "$config->plagiarism_plagkhal_apiurl/api/moodle/$config->plagiarism_plagkhal_key/settings";
            if (isset($cmid)) {
